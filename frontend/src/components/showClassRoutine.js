...
import { useDeleteClassRoutineMutation } from "../rtkq/services/userAuthApi";
...
const ShowClassRoutine = () => {
  ...
  if (
    Array.isArray(userData.teacherName) &&
    Array.isArray(userData.subjectName)
  ) {
    let maxLength = Math.max(
      userData.teacherName.length,
      userData.subjectName.length
    );
  ...
  return (
    <>
      <h1 className="text-center p-6 text-3xl font-semibold">
          Class Routine
      </h1>
              ...
              <div key={index} className="mb-4">
                <h2
                  ...
                  style={{
                    cursor: "pointer",
                    textDecoration:
                      selectedDay === index ? "underline" : "none",
                  }}
                ...
                {selectedDay === index && (
                  <div className="mt-4">
                    <table className="border-collapse w-3/4 text-center border border-blue-950 m-auto">
                      <tr className="">
                        <th
                          className="p-5 border border-slate-400"
                          rowSpan="2"
